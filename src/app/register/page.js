...
export default function Page(){
    ...
    const gotoDashboard = () => {
        if (emailValue === "" || passwordValue === "" || !isChecked || usernameValue === "") {
            ...
            if (usernameValue === "") {
                setShowUsernametip(true);  // Corrected to setShowPasswordtip
            }
            if (!isChecked) {
                setShakeText(true);
            }
            ...
            <div className="m-8 block  w-1/2 max-md:w-full">
                ...
                <div className="block mt-10 w-2/3 max-md:w-full">
                    <Link href="/" className="flex w-full shadow-xl h-11 justify-center text-center align-middle rounded-lg items-center">
                        ...
                        <p className="font-Ambit text-black font-bold">Register with linkdin</p>
                    </Link>
                </div>
                <div className="flex w-2/3 text-center mt-8 items-center justify-center max-md:w-full">
                    <p className="font-Ambit font-bold text-regal-grey">Already have an Account? </p>
                    <a href="/login" className="text-regal-blue"> Login</a>
                </div>
            </div>
            <div className="m-8 block w-1/2  max-md:hidden">
                <Image src="/image/robot.png" alt="robot" width={1000} height={1000} />
            </div>
        </div>
    );
}